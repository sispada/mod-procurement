<?php

namespace Module\Procurement\Http\Resources;

use Illuminate\Http\Resources\Json\JsonResource;
use Module\Procurement\Models\ProcurementWorkbio;
use Module\System\Http\Resources\UserLogActivity;

class WorkbioShowResource extends JsonResource
{
    /**
     * Transform the resource into an array.
     *
     * @param  \Illuminate\Http\Request  $request
     * @return array|\Illuminate\Contracts\Support\Arrayable|\JsonSerializable
     */
    public function toArray($request)
    {
        return [
            /**
             * the record data
             */
            'record' => ProcurementWorkbio::mapResourceShow($request, $this),

            /**
             * the page setups
             */
            'setups' => [
                'combos' => ProcurementWorkbio::mapCombos($request, $this),

                'icon' => ProcurementWorkbio::getPageIcon('procurement-workbio'),

                'key' => ProcurementWorkbio::getDataKey(),

                'logs' => $request->activities ? UserLogActivity::collection($this->activitylogs) : null,

                'softdelete' => $this->trashed() ?: false,

                'statuses' => ProcurementWorkbio::mapStatuses($request, $this),

                'title' => ProcurementWorkbio::getPageTitle($request, 'procurement-workbio'),
            ],
        ];
    }
}
