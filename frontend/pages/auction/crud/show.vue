<template>
  <form-show with-helpdesk>
    <template
      v-slot:default="{
        combos: { officers, methods, types, workunits },
        record,
      }"
    >
    ...
    >
      ...
      <v-row dense>
        ...
        <v-col
          cols="12"
          v-if="
            record.mode === 'PPBJ' && record.status === 'SUBMITTED' && isPOKJA
          "
        >
          <v-btn
            class="mt-3"
            :color="theme"
            block
            @click="completedAuction(record)"
            >KIRIM HASIL PENGADAAN</v-btn
          >
        </v-col>
...
export default {
  ...
  methods: {
    ...
    completedAuction: function (record) {
      this.$http(`procurement/api/auction/${record.id}/completed`, {
        method: "PUT",
        params: { ...record, _method: "PUT" },
      }).then(() => {
        this.$router.push({
